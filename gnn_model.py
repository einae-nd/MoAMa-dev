...
class GNN(torch.nn.Module):
    ...
    def forward(self, *argv):
        if len(argv) == 3:
            x, edge_index, edge_attr = argv[0].to(self.device), argv[1].to(self.device), argv[2].to(self.device)
        elif len(argv) == 1:
            data = argv[0]
            x, edge_index, edge_attr = data.x.to(self.device), data.edge_index.to(self.device), data.edge_attr.to(self.device)
        else:
            raise ValueError("unmatched number of arguments.")

        x = self.x_embedding1(x[:,0].to(self.device)) + self.x_embedding2(x[:,1].to(self.device))
